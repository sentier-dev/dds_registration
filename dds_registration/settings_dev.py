...
env = environ.Env(
    ...
    REGISTRATION_SALT=(str, 'OdUqhiXaWN'),  # Random salt for registration keys (to use in django_registration module)
)
...
# Use filters to generate assets from sources on-the-fly (like sass, see `COMPRESS_PRECOMPILERS` section below)
