...
def get_event_registration_form_context(
    request: HttpRequest, event_code: str, create_new: bool = False
):
    context = {
    ...
    try:
        ...
        if not create_new:
            ...
            msg_text = (
                'An active registration for the event "{}" already exists'.format(
                    event.title
                )
            )
            debug_data = {
    ...
    try:
        ...
        error_text = (
            'Got error while finding registration options for event "{}"'.format(
                event_code
            )
        )
        messages.error(request, error_text)
    ...
    try:
        ...
        reg_options_basic_checked_ids = list(
            set(checked_option_ids) & set(reg_options_basic_ids)
        )
...
def event_registration_form(
    ...
    context = get_event_registration_form_context(
        request, event_code=event_code, create_new=create_new
    )
...
def show_registration_form_success(
    request: HttpRequest, event_code: str, template: str
):
    user = request.user
    if not user.is_authenticated:
...
def calculate_total_registration_price(registration: Registration) -> int:
    ...
    options_price = reduce(
        lambda sum, opt: sum + opt.price if opt.price else sum, options, 0
    )
...
def send_event_registration_success_message(request: HttpRequest, event_code: str):
    ...
    email_body_template = (
        "dds_registration/event_registration_new_success_message_body.txt"
    )
    email_subject_template = (
        "dds_registration/event_registration_new_success_message_subject.txt"
    )
